...
- **Swipe navigation:** `useSwipeNavigation` hook enables horizontal swipe between the Dashboard and My Shows tabs, triggering a slide-in CSS animation on the `<main>` element. On a ShowDetail page, swipe is handled by `ShowDetail` itself (not the tab hook) and moves to the previous/next show in the same alphabetical sort order as the My Shows list; the slide animation runs on the page container.
