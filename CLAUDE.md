...
- **Bookmark (`current_season`/`current_episode`):** A user's progress marker, computed as the first episode after their highest watched one (or `NULL` = caught up). The shared `recomputeBookmark` helper (`functions/api/episodes/_bookmark.ts`) is the single source of truth and must be called everywhere the inputs change — both on watch-state changes (`PUT /api/episodes/:id`) **and** after `POST /api/shows/sync` pulls in new episodes. Skipping the sync recompute leaves a stale `NULL` bookmark when a new episode airs, so My Shows shows "Caught up" while the Dashboard (which falls back to the first unwatched aired episode) shows the real next episode. See [#24](https://github.com/dylano/streamd/issues/24).
- **Swipe navigation:** `useSwipeNavigation` hook enables horizontal swipe between the Dashboard and My Shows tabs, triggering a slide-in CSS animation on the `<main>` element. On a ShowDetail page, swipe is handled by `ShowDetail` itself (not the tab hook) and moves to the previous/next show; the slide animation runs on the page container. The swipe order matches the list the show was opened from: the Dashboard passes its "Next Up" order (by air date) via React Router navigation `state` (`{ swipeOrder: number[] }`), which `ShowDetail` preserves across swipes; without it (e.g. opened from My Shows) it falls back to the alphabetical My Shows order. Swipes use `navigate(..., { replace: true })` so they don't accumulate history — the back button returns to wherever the show was opened from. Note: the touch-listener effect depends on `show?.id` so it (re)attaches when the real page element mounts after the show loads.
