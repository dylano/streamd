...
// GET /api/tmdb/show/:id/watch-providers - Get streaming providers from TMDB
export const onRequestGet: PagesFunction<Env, "id"> = async (context) => {
  const id = context.params.id;

  const cacheKey = `watch-providers:${id}`;
  ...
  const tmdbUrl = `${TMDB_BASE}/tv/${id}/watch/providers?api_key=${context.env.TMDB_API_KEY}`;
